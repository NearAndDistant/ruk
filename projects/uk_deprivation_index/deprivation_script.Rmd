...
con_ward2 <- read_csv(here::here("projects/uk_deprivation_index/Ward_to_Westminster_Parliamentary_Constituency_to_Local_Authority_District_(December_2019)_Lookup_in_the_United_Kingdom.csv"))
...
# check all constituencies caught
# all_pop_raw %>%
#   left_join(ukdi_index %>% distinct(PCON20CD), by = c(con_code = "PCON20CD")) %>%
#   view()

ukdi_cons %>% anti_join(all_pop_raw, by = c(PCON20CD = "con_code")) %>% view()
