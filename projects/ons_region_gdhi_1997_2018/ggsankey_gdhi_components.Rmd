...
prep_NUTS1 %>%
  group_by(region_name) %>%
  mutate(id2 = str_pad(6:18, width = 2, pad = "0")) %>%
...
regions <- 
  tibble(
    ...
    group_by(source) %>%
    filter(str_detect(id, ".06|.07|.08|.09|.13|.14"))
...
regions <- 
  regions %>% 
  group_by(region_name) %>%   
  mutate(id2 = str_pad(1:5, width = 2, pad = "0")) %>%
  mutate(id = paste(id , id2, sep = ".")) %>%
  select(-id2) %>%
  select(id , everything())
...
"London"            = "Pale Green"      = "#dff2bd"        
"East Of England"   = "Dark Orange"     = "#fcc0a6"              
"Northern Ireland"  = "Dark Green"      = "#076543"                               

```{r}

link_color <- 
JS('d3.scaleOrdinal()
    .domain(["Purple", "Orange",  "Blue",    "Brown",   "Yellow",  "Olive",   "Lime",    "Pink",    "Grey", "Pale Green","Dark Orange", "Dark Green" ])
    .range(["#e2d8f4", "#ffefd9", "#dff3f5", "#efead7", "#ffffbf", "#bfc9ad", "#d1ee70", "#ffc9c9", "#bfc9c7", "#dff2bd", "#fcc0a6", "#076543"])')
...
networkD3_table %>%
  mutate(
    ...
    region_name == "London"           ~ "Pale Green" ,
    region_name == "East Of England"  ~ "Dark Orange",
    region_name == "Northern Ireland" ~ "Dark Green" ))
...
list(
  ...
  links = data.frame(source =        networkD3_table$source_id,
                     ...
                     group  =        networkD3_table$region_name,
...
sankeyNetwork(Links = nd3_gdhi$links, Nodes = nd3_gdhi$nodes , 
              ...
              units = "£", fontSize = 14, fontFamily = "Arial" , nodeWidth = 30, colourScale = link_color, LinkGroup = "color",
